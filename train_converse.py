...
curriculum_next_out_dir = 'out_next' # output directory for next curriculum step
# model
n_layer = 12
n_head = 12
n_embd = 768
...
class Agent():
    ...
    @torch.no_grad()
    def label_dataset(self, input_path, label_path, output_path, split, batch_size=2000):
        """
        ...
        prepare(output_path, self.config['out_dir'], file_name_suffix, split=split)
        if self.config['prune_dataset']:
    ...
    @torch.no_grad()
    def generate_hard_dataset(self, input_dir, output_dir, batch_size=2000):
        for split in ['train', 'val']:
            # label paths
            input_path0 = os.path.join(input_dir, f'{split}0_round0.txt')
            input_path1 = os.path.join(input_dir, f'{split}1_round0.txt')

            # input paths
            agent_path = os.path.join(input_dir, f'{split}{self.id}_round{self.round}.txt')

            # output paths
            hard_path0 = os.path.join(output_dir, f'{split}0_round0.txt')
            hard_path1 = os.path.join(output_dir, f'{split}1_round0.txt')

            # load input data
            with open(input_path0, 'r') as f:
                input_lines0 = f.readlines()
            with open(input_path1, 'r') as f:
                input_lines1 = f.readlines()
            with open(agent_path, 'r') as f:
                agent_lines = f.readlines()
            assert len(input_lines0) == len(input_lines1) == len(agent_lines), f"number of lines in input_lines0, input_lines1, and agent_lines must be the same"
            
            num_batches = len(input_lines0) // batch_size
            for batch_idx in tqdm.tqdm(range(num_batches)):
                start_line_idx = batch_idx * batch_size
                agent_lines_batch = agent_lines[start_line_idx:start_line_idx + batch_size]
                starts = [agent_line[:self.block_size] for agent_line in agent_lines_batch]
                start_ids = [self.encode(start) for start in starts]
                start_ids = torch.tensor(start_ids, dtype=torch.long, device=self.config['device'])
                ...
                easy_example_indices = [] # keep track of indices of easy examples
                hard_lines0 = []
                hard_lines1 = []
                for i, prob in enumerate(answer_probs):
                    example_idx = start_line_idx + i
                    # if answer_probs has prob 1 on the correct answer, skip it
                    correct_answer = agent_lines_batch[i].rstrip('\n')[-1]
                    correct_idx = self.config['answer_tokens'].index(correct_answer)
                    prob_correct_answer = prob[0, correct_idx].item()
                    if prob_correct_answer > 0.999:
                        easy_example_indices.append(example_idx)
                    else: # append to hard dataset
                        hard_lines0.append(input_lines0[example_idx])
                        hard_lines1.append(input_lines1[example_idx])
                
                num_hard_examples = batch_size - len(easy_example_indices)
                if len(easy_example_indices) > num_hard_examples * (0.1/0.9):
                    easy_examples_sample = random.sample(easy_example_indices, num_hard_examples * (0.1/0.9))
                else:
                    easy_examples_sample = easy_example_indices
                hard_lines0.extend([input_lines0[idx] for idx in easy_examples_sample])
                hard_lines1.extend([input_lines1[idx] for idx in easy_examples_sample])
                # shuffle hard dataset lines
                random.shuffle(hard_lines0)
                random.shuffle(hard_lines1)
                # write to hard dataset
                with open(hard_path0, 'a', encoding="utf-8") as out:
                    out.writelines(hard_lines0)
                with open(hard_path1, 'a', encoding="utf-8") as out:
                    out.writelines(hard_lines1)
            
            # print number of examples in hard dataset
            num_examples = len(input_lines0)
            with open(hard_path0, 'r') as f:
                num_hard_examples0 = len(f.readlines())
            with open(hard_path1, 'r') as f:
                num_hard_examples1 = len(f.readlines())
            fraction_hard_examples0 = num_hard_examples0 / num_examples
            fraction_hard_examples1 = num_hard_examples1 / num_examples
            print(colored(f"Number of examples in hard {split} dataset: {num_hard_examples0}", 'light_blue'))
            print(colored(f"Number of examples in hard {split} dataset: {num_hard_examples1}", 'light_blue'))
            print(colored(f"Percentage of examples in hard {split} dataset: {fraction_hard_examples0 * 100}%", 'light_blue'))
            print(colored(f"Percentage of examples in hard {split} dataset: {fraction_hard_examples1 * 100}%", 'light_blue'))
            if self.config['wandb_log'] and wandb.run is not None:
                wandb.log({f"data/{split}_fraction_hard_examples0": fraction_hard_examples0})
                wandb.log({f"data/{split}_fraction_hard_examples1": fraction_hard_examples1})
            
            # prepare output files
            prepare(hard_path0, output_dir, f'0_round0', split=split)
            prepare(hard_path1, output_dir, f'1_round0', split=split)
        ...
        for split in ['train', 'val']:
            ...
            if self.collaborator_model is not None:
                ...
                if self.config['post_hoc_calibrate']:
                    calibrator_losses = torch.zeros(self.config['eval_iters'])
            ...
            for k in range(self.config['eval_iters']):
                ...
                if self.collaborator_model is not None:
                    ...
                    if self.config['post_hoc_calibrate']:
                        calibrator_losses[k] = calibrator_loss.item()
            ...
            if self.collaborator_model is not None:
                ...
                if self.config['post_hoc_calibrate']:
                    out[split + '_calibrator_loss'] = calibrator_losses.mean()
    ...
    def train(self):
        ...
        while True:
            ...
            if self.iter_num % self.config['eval_interval'] == 0 and self.master_process:
                ...
                if self.config['wandb_log']:
                    ...
                    if self.collaborator_model is not None:
                        ...
                        if self.config['post_hoc_calibrate']:
                            log_data.update({
                                "train/calibrator_loss": losses['train_calibrator_loss'],
                                "val/calibrator_loss": losses['val_calibrator_loss'],
            ...
            if self.iter_num == self.config['max_iters']:
                ...
                for split in ['train', 'val']:
                    if self.round == 0:
                        input_path = os.path.join(self.data_dir, f'{split}{self.id}_round{self.round}.txt')
                    else:
                        input_path = os.path.join(self.config['out_dir'], f'{split}{self.id}_round{self.round}.txt')
                    label_path = os.path.join(self.data_dir, f'{split}{next_agent_id}_round{0}.txt') # label on original dataset
                    output_path = os.path.join(self.config['out_dir'], f'{split}{next_agent_id}_round{self.round+1}.txt')
                    print(colored(f"labeling {split} dataset for round {self.round+1} agent {next_agent_id} =================================================", 'light_green'))
            ...
            if self.config['generate_hard_dataset'] and self.round == self.config['num_rounds'] - 1 and self.iter_num == self.config['max_iters']: 
                # generate hard dataset at the last round for next curriculum step
                print(colored(f"generating hard dataset for next curriculum step, saving to {self.config['curriculum_next_out_dir']} =================================================", 'light_green'))
                start_time = time.time()
                self.generate_hard_dataset(self.config['out_dir'], self.config['curriculum_next_out_dir'])
                end_time = time.time()
                print(colored(f"time taken: {end_time - start_time:.2f} seconds", 'light_green'))
