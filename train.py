...
def get_batch(split):
    ...
    # ix is multiples of 17 and has batch size batch_size
    starting_indices = torch.arange(0, len(data), 17)
    ix = torch.randint(len(starting_indices), (batch_size,))
...
model_args = dict(n_layer=n_layer, n_head=n_head, n_embd=n_embd, block_size=block_size,
                  bias=bias, vocab_size=None, dropout=dropout, prefix_size=prefix_size, calibrate=calibrate, multiplier=multiplier, causal=causal) # start with model_args from command line
if init_from == 'scratch':
    ...
    for k in ['n_layer', 'n_head', 'n_embd', 'block_size', 'bias', 'vocab_size', 'prefix_size', 'causal']:
